...
if __name__ == '__main__':
	...
	amp_data = util.pickle_load('./data/S8/amp_data_dspikes.pickle')
...
# 	
# 	plt.sca(axBoxplot)
# 	stat, pval = test(np.array(amp_data[0:7,0]),np.array(amp_data[7:,0]), equal_var=False)
...
# 	
# 	plt.sca(axBoxplot)
# 	stat, pval = test(np.array(amp_data[0:7,1]),np.array(amp_data[7:,1]), equal_var=False)
...
# 	
# 	
# 	# ===== Amplitude difference (d-s) (no other var) =====
# 	fig, axes = plt.subplot_mosaic('AAB', dpi=_RES_DPI)
# 	axScatter = axes['A']
# 	axBoxplot = axes['B']
# 	plt.sca(axScatter)
...
# 	
# 	plt.sca(axBoxplot)
# 	stat, pval = test(np.array(amp_data[0:7,2]),np.array(amp_data[7:,2]), equal_var=False)
