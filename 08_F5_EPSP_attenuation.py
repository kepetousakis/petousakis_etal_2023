...
if __name__ == '__main__':
	...
	(amplitudes, widths, delays, delays_dV) = util.pickle_load('./Data/F5/EPSP_validation_results.pickle')
	...
	try:	
		amp_data = util.pickle_load('./Data/F5/amp_data_EPSPs.pickle')
	except:
...
# 	plt.scatter(range_basals, amp_data[0:7,0],c='b', label='basal')
# 	plt.scatter(range_apicals, amp_data[7:,0],c='r', label='apical')
# 	plt.legend()
# 	
# 	plt.sca(axBoxplot)
# 	stat, pval = test(np.array(amp_data[0:7,0]),np.array(amp_data[7:,0]), equal_var=assume_equal_variances)
...
# 	plt.scatter(range_basals, amp_data[0:7,1],c='b', label='basal')
# 	plt.scatter(range_apicals, amp_data[7:,1],c='r', label='apical')
# 	plt.legend()
# 	
# 	plt.sca(axBoxplot)
# 	stat, pval = test(np.array(amp_data[0:7,1]),np.array(amp_data[7:,1]), equal_var=assume_equal_variances)
...
# 	
# 	plt.sca(axBoxplot)
# 	stat, pval = test(np.array(amp_data[0:7,2]),np.array(amp_data[7:,2]), equal_var=assume_equal_variances)
# 	print(f'T-test statistic (basal vs apical attenuation): {stat:.04f} | p-value: {pval:.06f} | {"NS" if pval>=0.05 else "*"}')
# 	plt.title(f'T-test statistic (basal vs apical attenuation):\n {stat:.04f} | p-value: {pval:.06f} | {"NS" if pval>=0.05 else "*"}')
...
# 			trace = np.loadtxt(f'./data/attenuation/EPSP/dendrite_{tgt}_nstim_{thr}/{compartment_names[comp]}.dat')
...
# 		plt.text(0, attenuation_trajectories[i][-1]+(25/40), f'{attenuation_trajectories[i][-1]:.02f}', fontsize=14)
# 				
# 	
...
# 			plt.text(0, attenuation_trajectories[j][-1]+(25/40), f'{attenuation_trajectories[j][-1]:.02f}', fontsize=14)
# 			
