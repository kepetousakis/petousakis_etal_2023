...
# 				(stat, pval) = test(data[i][feature], data[j][feature])
				(stat, pval) = test(data[i], data[j], equal_var=_eq_var)
				print(f'Testing {comparator} against {comparand}. Results: Statistic {stat:2.4f}, p-value {pval:2.8f} | Verdict: \t {verdict(pval)}')
				results[comparator][comparand] = pval
				res_array[i][j] = pval
				if not _BONFERRONI:
...
filepath = './Data/F4/spike_survival_verdicts_revisions_syn_intv.pickle'
directory = './Data/F4/'
filepath2 = './Data/F4/pre_intervention_spike_timings_revisions_syn_intv.pickle'
affix = '_syn_intv'
